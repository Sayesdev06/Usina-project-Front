...
function ListElementQuantity(props) {
  ...
  const { enqueueSnackbar } = useSnackbar();
    ...
    const handleConfirmDelete = () => {
      const data = {
        ...
        handleClickVariant: (msg, val) => {
          ...
          props.allDetailsProduct()
        }
      }
      dispatch(actionCreator.deleteElement(data))
    }
    ...
    const handleConfirmUpdate = () => {
      ...
      dispatch(actionCreator.updateElement(data))
    }
        ...
        <Table aria-label="spanning table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="left">ID-Commande de vente</TableCell>
          ...
          {productPieces&& productPieces.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((piece,key)=>(
            <TableRow key={key}>
              ...
              <TableCell align="left">{piece.value}</TableCell>
              <TableCell align="left">
              {/* {piece.sellOrderId ?
             <a className='a-href' onClick={() => goToAddSellOrder(piece.sellOrderId,"details")}>{piece.sellOrderId}</a>
              : "Non utilisé"} */}
              {piece.isUsed ?
                <span className='used-element'>Utilisé</span>:<span className='not-used-element'>Non utilisé</span>
              }
                </TableCell>

                <TableCell sx={{ color: "#3a3541ad" }} >
                        <PencilOutline sx={{ fontSize: '1.2rem', cursor: "pointer", ml: 2 }} 
                        onClick={()=>{setElementSelected(piece);setOpenModalUpdateElement(true)}} />
                        <DeleteOutline sx={{ fontSize: '1.2rem', cursor: "pointer", ml: 2 , color:"red"}}
                        onClick={()=>{setElementSelected(piece);setOpenModalDeleteElement(true)}}  />
                      </TableCell>
...
export default ListElementQuantity